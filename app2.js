...
const server = http.createServer((req, res) => {
    ...
    //  console.log("headers: ", res.headers);
        ...
        res.on('data', function(d) {
            ...
            var requests = lastMatchInformations.last_match.players.map(retrieveUserData);
            
            users = Promise.all(requests);
...
*/
            users.then(data => process.stdout.write('FINAL RESULT: ' + JSON.stringify(data)));
...
var retrieveUserData = function(userFromApi) {
    return new Promise(function (resolve, reject) {
        var optionsget = {
            ...
            path : '/api/leaderboard?game=aoe2de&steam_id=' + userFromApi.steam_id, // the rest of the url with parameters if needed
            method : 'GET' // do GET
        };
